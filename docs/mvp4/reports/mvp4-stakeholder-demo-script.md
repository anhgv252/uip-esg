...
**Tell:** "After MVP4: **~$0.187/day** (extrapolated 24h) — **>260x cost reduction** from $50/day. Same alerts, fraction of the spend. Measured live on staging 2026-06-18: AI pipeline RUNNING end-to-end (Flink batching → `ai.district.aggregations` → `DistrictAggregationConsumer` → Claude Haiku), cache hit rate 56.1%, 65 real API calls @ $0.000130/call. Official 24h cost measurement runs during the Aug pilot."
...
|---|---|---|---|
| AI cost/day @ 10K sensors | < $1.00 | **✅ ~$0.187/day** (extrapolated) | Staging re-run 2026-06-18: pipeline RUNNING, cache hit 56.1%, 65 real Claude calls @ $0.000130/call, claude-haiku-4-5. Official 24h during Aug pilot |
...
| Regression | ≥1,500 tests, 0 fail | **✅ 1,726 tests, 0 fail** | BUILD SUCCESSFUL 2026-06-18 |
| 1000 VU JMeter | p95<500ms, <1% err | **✅ p95=6ms, 0% err, 596 RPS** | JMeter rerun3 2026-06-18: 214,670 samples, 3 endpoints symmetric. (ConstantThroughputTimer pace 600 req/s + backend mem_limit 768m→1536m) |
...
| OWASP | 0 crit/high | **✅ 0 CVE CVSS≥7** | dependencyCheckAggregate 2026-06-18 (gRPC 1.71 + protobuf 3.25.5) |
...
- [ ] **Kafka state clean** (lesson `feedback_mvp4_kafka_reset_runbook`) — verify `kafka-consumer-groups --describe` shows LAG=0 across all MVP4 consumer groups, no `UNKNOWN_TOPIC_OR_PARTITION` warns in backend logs. Both Flink jobs (`DistrictAggregationJob`, `IncidentCorrelationJob`) re-submitted and RUNNING with source+sink vertices bound. Reset procedure in `infrastructure/README-ops.md` § "Switching single ↔ HA compose".
- [x] G1 value filled: **~$0.187/day** (extrapolated, staging re-run 2026-06-18)
- [x] G5 value filled: **p95=6ms, error=0.0000%, RPS=596.1** (rerun3 2026-06-18)
- [x] Regression: **1,726 tests, 0 fail** (2026-06-18)
