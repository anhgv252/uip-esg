...
## 7. Sprint 4 — AI Workflow + UAT Preparation (09/04 → 23/05/2026) 🔄 RESTRUCTURED
...
✅ S4-10: Admin CRUD API hoạt động — WorkflowConfigController (list/get/create/update/disable/test) + 6 unit tests
✅ Old trigger classes disabled (không xóa — rollback ready) — 5 classes @Component commented
☐ S4-10: Integration test 7 scenarios qua generic triggers
☐ S4-10: Admin Console UI (FE)
☐ S4-00: AI Workflow Dashboard hiển thị 7 scenarios + instances
...
### Sprint 4 — 🔄 RESTRUCTURED (09/04 → 23/05/2026)
...
- Trigger code in S4-04 (already done) → disabled, not thrown away entirely (delegates reused)
- **Total net saving: ~2 SP** + avoids double-work (code specific triggers then code generic to replace)
